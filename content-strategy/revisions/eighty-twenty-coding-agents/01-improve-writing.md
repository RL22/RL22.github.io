...
The standard response is prompt inflation. Developers write 300-line system prompts stuffed with rules, warnings, and formatting demands, hoping more instructions will force the model into line. It rarely works. Monolithic prompts degrade attention, waste tokens, and accumulate contradictions.

When Matt Pocock’s agent skills repository passed 214,000 GitHub stars, it became one of the most popular AI developer projects in history. But if you look past the star count and examine the codebase, the real lesson is not that you need dozens of complex tools. It is that 80% of agentic reliability comes from four lightweight, structural constraints. In fact, one of the most effective skills in the entire repository is only 22 lines of markdown.

Here is the 80/20 of AI coding agents: the four practices that actually make agents reliable in production, and the one anti-pattern you should delete today.
...
Matt Pocock’s `grill-me` skill solves this in 22 lines. Instead of letting the agent write code, `grill-me` forces it to interview the developer before touching the codebase.

The skill structures the conversation as a decision tree:
...
- Questions arrive in numbered batches with recommended defaults.
...
Each ticket is self-contained: it specifies the exact files to modify, the acceptance criteria, and the verification commands. If a ticket cannot be planned, implemented, and verified within a fresh context window, it is too large and must be split.
