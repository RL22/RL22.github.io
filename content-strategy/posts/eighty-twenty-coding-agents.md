...
| Dimension | High-Fidelity Prompting (The Trap) | Low-Fidelity Alignment (The 80/20) |
|---|---|---|
| **Turn 1 Output** | 500 lines of unvetted code | 3–5 targeted architectural questions |
| **Feedback Loop** | Expensive (line-by-line diff review) | Cheap (1-word shorthand answers) |
| **Correction Cost** | High (refactor files, delete code) | Zero (adjust text decision boundary) |
| **Context Load** | Heavy token burn from code dumps | Minimal token burn on plain text |
...
| Prompt Anti-Pattern (No-Op) | Why It Fails | The Architectural Fix |
|---|---|---|
| *"Be thorough and don't make mistakes."* | Subjective; model cannot compute it | Delete entirely. |
| *"Remember to run tests before committing."* | Relies on probabilistic memory | Write a pre-commit hook or CI gate. |
| Rules duplicated across 3 prompt files | Causes synchronization drift | Single source of truth in one markdown file. |
